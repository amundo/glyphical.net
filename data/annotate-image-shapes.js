...
const SVG_DIR = '/Users/patrickhall/Sites/Languages/egyptian/jsesh/jseshGlyphs/src/main/resources/jseshGlyphs/'
...
let  classifyHieroglyphs = async () => {
  ...
  const svgFiles = files
    .filter(file => file.name.endsWith('.svg') && hieroglyphDb.hieroglyphs
...
let allSvgs= ''
  ...
  const dimensions = await Promise.all(svgFiles.map(async file => {
    ...
    return { content, aspect, gardiner: file.name.replace('.svg', ''), width, height };
  }))

  dimensions.sort((a,b) => 
    a.aspect < b.aspect ? 1 : -1
  )
  .forEach(h => allSvgs += `<h2>${h.gardiner}: ${h.content} ${h.aspect}</h2>`)
  ...
  dimensions.forEach(dimension => {
    const hieroglyph = hieroglyphDb.hieroglyphs.find(h => h.gardiner === dimension.gardiner);
    if (dimension.height < (2/3*averageHeight) && dimension.width < (2/3*averageWidth)) {
