...
let uniAndBiLiteral = uniliterals.filter(uniliteral => {
  return biliterals.find(biliteral => biliteral.hieroglyph == uniliteral.hieroglyph)
})

let biAndTriLiteral = biliterals.filter(biliteral => {
  return triliterals.find(triliteral => triliteral.hieroglyph == biliteral.hieroglyph)
})

let uniAndTriLiteral = uniliterals.filter(uniliteral => {
  return triliterals.find(triliteral => triliteral.hieroglyph == uniliteral.hieroglyph)
})
